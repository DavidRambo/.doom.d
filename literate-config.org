...
I often want to use Writeroom simply to set ~visual-fill-column~ to ~80~ (but with variable-pitch it works out to about 100 actual characters) and to set ~visual-fill-column-center-text~ to ~t~ without closing other windows.
